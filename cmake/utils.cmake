# Get DungineX version from DungineX/include/DgeX/Version.h" and 
# put it in DGEX_VERSION
function(dgex_extract_version)
    file(READ "${CMAKE_CURRENT_LIST_DIR}/DungineX/include/DgeX/Version.h" file_contents)
...
# Get DungineX Physics version from DungineX/include/DgeX/Physics/Version.h" and 
# put it in DPHX_VERSION
function(dphx_extract_version)
    file(READ "${CMAKE_CURRENT_LIST_DIR}/DungineX/include/DgeX/Physics/Version.h" file_contents)
    string(REGEX MATCH "DPHX_VERSION_MAJOR ([0-9]+)" _ "${file_contents}")
    if(NOT CMAKE_MATCH_COUNT EQUAL 1)
        message(FATAL_ERROR "Could not extract major version number")
    endif()
    set(ver_major ${CMAKE_MATCH_1})

    string(REGEX MATCH "DPHX_VERSION_MINOR ([0-9]+)" _ "${file_contents}")
    if(NOT CMAKE_MATCH_COUNT EQUAL 1)
        message(FATAL_ERROR "Could not extract minor version number")
    endif()

    set(ver_minor ${CMAKE_MATCH_1})
    string(REGEX MATCH "DPHX_VERSION_PATCH ([0-9]+)" _ "${file_contents}")
    if(NOT CMAKE_MATCH_COUNT EQUAL 1)
        message(FATAL_ERROR "Could not extract patch version number")
    endif()
    set(ver_patch ${CMAKE_MATCH_1})

    set(DPHX_VERSION_MAJOR ${ver_major} PARENT_SCOPE)
    set(DPHX_VERSION_MINOR ${ver_minor} PARENT_SCOPE)
    set(DPHX_VERSION_PATCH ${ver_patch} PARENT_SCOPE)
    set(DPHX_VERSION "${ver_major}.${ver_minor}.${ver_patch}" PARENT_SCOPE)
endfunction()
